...
[5/6/2024]Time up
LeetCode's computation doesn't match local result. Why?
O(h*w)/O(h*w)
*/
class Solution {
    int[][] heights;
    int h;
    int w;
    Boolean[][] canFlowPacific;
    Boolean[][] canFlowAtlantic;
    final Set<String> trace = new HashSet<>();

    public List<List<Integer>> pacificAtlantic(int[][] heights) {
        this.heights = heights;
        h = heights.length;
        w = heights[0].length;
        canFlowPacific = new Boolean[h][w];
        canFlowAtlantic = new Boolean[h][w];
        List<List<Integer>> ans = new LinkedList<>();
        for (int i = 0; i < h; i++) {
            for (int j = 0; j < w; j++) {
                if (testPacific(i, j) && testAtlantic(i, j)) ans.add(List.of(i, j));
            }
        }
        return ans;
    }

    private boolean testPacific(int i, int j) {
        if (canFlowPacific[i][j] != null) {
            return canFlowPacific[i][j];
        }
        ...
        String currPoint = String.format("%s,%s", i, j);
        if (trace.contains(currPoint)) return false;
        trace.add(currPoint);
        // DFS
        int currHeight = heights[i][j];
        if (0 < i && heights[i - 1][j] <= currHeight && testPacific(i - 1, j)) {
            canFlowPacific[i][j] = true;
            trace.remove(currPoint);
            return true;
        }
        if (i < h - 1 && heights[i + 1][j] <= currHeight && testPacific(i + 1, j)) {
            canFlowPacific[i][j] = true;
            trace.remove(currPoint);
            return true;
        }
        if (0 < j && heights[i][j - 1] <= currHeight && testPacific(i, j - 1)) {
            canFlowPacific[i][j] = true;
            trace.remove(currPoint);
            return true;
        }
        if (j < w - 1 && heights[i][j + 1] <= currHeight && testPacific(i, j + 1)) {
            canFlowPacific[i][j] = true;
            trace.remove(currPoint);
            return true;
        }
        canFlowPacific[i][j] = false;
        trace.remove(currPoint);
        return false;
    }

    private boolean testAtlantic(int i, int j) {
        if (canFlowAtlantic[i][j] != null) {
            return canFlowAtlantic[i][j];
        }
